...
pub fn main() {
    ...
    let mut keyboard_devices: Vec<KeyboardDevice> = evdev::enumerate()
        .filter(|device| check_keyboard(&device))
        .map(|device| KeyboardDevice::new(device))
        .collect();
    ...
    loop {
        nix::poll::poll(&mut pollfds, poll_timeout).unwrap();
        for (i, _) in pollfds
            .iter()
            .enumerate()
